
from generate_diversity_sets import generate_diversity_sets
import numpy as np

from warehouse_location import warehouse_location

per_list = [110, 115, 120]
for per in per_list:
    print("Percentage = "+str(per)+"%")
    seed = 20
    np.random.seed(seed)
    nw = 8
    ns = 10
    per = per
    bool_list, set_list, value_list = warehouse_location(nw=nw, ns=ns, per=per, save=True, seed=seed)
    k = 20
    optimal_index = np.argmax(value_list)

    base_path = "experiments/warehouse/n_solutions/diversity_sets/randomseed_"+str(seed)+"_ns"+str(ns)+"_nw"+str(nw)+"_k"+str(k)+"_per"+str(per)+"_"
    mdl, hd = generate_diversity_sets(set_list=set_list, bool_list=bool_list, k=k, optimal_index=optimal_index,
                                      base_path=base_path, save=True, fitness_list=value_list, tie_breaker=False)







